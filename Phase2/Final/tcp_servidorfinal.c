...
void criar_admin() {
    char nome[50] = "admin";
    char password[50] = "admin"; // Corrigido o nome da variável
    int encontrado = 0;
    char linha[110], user_file[50], pass_file[50];
    
    // "a+" permite ler o ficheiro e escrever no final (append)
    FILE *fp = fopen("utilizadores.txt", "a+");
    
    if (fp == NULL) {
        printf("Log: Erro ao abrir/criar ficheiro de utilizadores.\n");
        return;
    }

    // Volta ao início do ficheiro para realizar a leitura/busca
    rewind(fp);

    while (fgets(linha, sizeof(linha), fp) != NULL) {
        // Tenta ler o formato usuario:senha
        if (sscanf(linha, "%49[^:]:%49[^:]", user_file, pass_file) == 2) {
            if (strcmp(user_file, nome) == 0) {
                encontrado = 1;
                break;
            }
        }
    ...
    // Se não encontrou, o ponteiro já está no fim devido ao modo "a+"
    if (!encontrado) {
        fprintf(fp, "%s:%s:%s:%s\n", nome, password,"1","1");
    }
...
void login(int client_fd) {
    char username[50], password[50],verif_file[50], buffer[BUF_SIZE];
    int nread;
    ...
    while (fgets(linha, sizeof(linha), fp) != NULL) {
        if (sscanf(linha, "%49[^:]:%49[^:]:%49[^:]", user_file, pass_file,verif_file) == 3) {
            if (strcmp(user_file, username) == 0 && strcmp(pass_file, password) == 0 && strcmp(verif_file,"1") == 0) {
                encontrado = 1;
                break;
            }
        }
...
void handle_commands(int client_fd, const char *username) {
    ...
    while ((nread = read(client_fd, buffer, BUF_SIZE - 1)) > 0) {
        ...
        // ---- F4: CHECK INBOX (Recuperar mensagens guardadas) ---- 
            ...
            FILE *f_msg = fopen(filename, "r");
            if (f_msg == NULL) {
                write(client_fd, "Nao tem mensagens novas.\n>> ", 28);
            } else {
                char inbox_content[BUF_SIZE] = "\n--- A SUA INBOX ---\n";
                char linha_msg[512];
                while (fgets(linha_msg, sizeof(linha_msg), f_msg)) {
