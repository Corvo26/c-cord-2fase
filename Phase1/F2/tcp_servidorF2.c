...
void login(int client_fd) {
    ...
    while (fgets(linha, sizeof(linha), fp)) {
        ...
        sscanf(linha, "%[^:]:%s", user_file, pass_file);
        
        if (strcmp(username, user_file) == 0 && strcmp(password, pass_file) == 0) {
            autenticado = 1;
            break;
        }
    }
    ...
    if (autenticado) {
        write(client_fd, "Login bem-sucedido!\n", 21);
        printf("Log: Utilizador autenticado: %s\n", username);
    } else {
        write(client_fd, "Credenciais invalidas.\n", 24);
        printf("Log: Falha de autenticacao para: %s\n", username);
    }
}
...
void registo(int client_fd) {
    ...
    FILE *fp = fopen("utilizadores.txt", "r");
    if (fp != NULL) {
        char linha[100], user_file[50], pass_file[50];
